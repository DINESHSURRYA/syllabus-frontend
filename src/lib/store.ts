...
export function normalizeSyllabusToStoreData(raw: any): SyllabusData {
  ...
  const units: UnitItem[] = rawUnits.map((u: any, uIdx: number) => {
    ...
    const unitHours = String((u.learningHours !== undefined && u.learningHours !== null && u.learningHours !== '') ? u.learningHours : ((u.hours !== undefined && u.hours !== null && u.hours !== '') ? u.hours : 9));
    ...
    const topics: TopicItem[] = rawTopics.map((t: any) => {
      ...
      return {
        ...
        hierarchyReason: typeof t === 'object' ? (t.hierarchyReason || t.hourAllocationReason || t.description) : undefined,
      };
    });
  ...
  return {
    ...
    course: {
      ...
      objectives: Array.isArray(raw.objectives) ? raw.objectives : (Array.isArray(courseObj.objectives) ? courseObj.objectives : []),
      outcomes: Array.isArray(raw.outcomes) ? raw.outcomes : (Array.isArray(courseObj.outcomes) ? courseObj.outcomes : []),
    },
    units,
    ...
    reference_books: Array.isArray(raw.reference_books) ? raw.reference_books : (Array.isArray(raw.references) ? raw.references : []),
...
export const useSyllabusStore = create<SyllabusStoreState>((set) => ({
  ...
  setExtractionProgress: (progress) =>
    set((state) => ({
      extractionState: { ...state.extractionState, ...progress },
    })),
  ...
  setSyllabus: (data) => {
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem('active_saved_syllabus', JSON.stringify(data));
      } catch (e) {
        console.error('Failed to save syllabus to localStorage:', e);
      }
    }
    set({ syllabus: data });
  },
  ...
  updateUnit: (unitIndex, fields) =>
    set((state) => {
      const updatedUnits = [...state.syllabus.units];
      updatedUnits[unitIndex] = { ...updatedUnits[unitIndex], ...fields };
      return {
        syllabus: { ...state.syllabus, units: updatedUnits }
      };
    }),
  ...
  reorderUnits: (startIndex, endIndex) =>
    set((state) => {
      ...
      // Re-index unit_number
      const reindexed = result.map((unit, idx) => ({ ...unit, unit_number: idx + 1 }));
      return { syllabus: { ...state.syllabus, units: reindexed } };
    }),

  addTopic: (unitIndex, topic) =>
    set((state) => {
      const updatedUnits = [...state.syllabus.units];
      updatedUnits[unitIndex].topics.push(topic);
      return { syllabus: { ...state.syllabus, units: updatedUnits } };
    }),

  updateTopic: (unitIndex, topicIndex, fields) =>
    set((state) => {
      const updatedUnits = [...state.syllabus.units];
...
// Asynchronously rehydrate syllabus from localStorage on client side after initial React hydration
if (typeof window !== 'undefined') {
  setTimeout(() => {
    try {
      const stored = localStorage.getItem('active_saved_syllabus');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && Array.isArray(parsed.units) && parsed.units.length > 0) {
          useSyllabusStore.setState({ syllabus: parsed });
        }
      }
    } catch (e) {
