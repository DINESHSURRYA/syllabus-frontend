...
export default function ReportScreen({ propReport }: ReportScreenProps) {
  const { report: storeReport, resetSession } = useEvaluatorStore();
  ...
  if (!report) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-full min-h-screen p-6">
        <h2 className="text-xl text-error mb-4">No report data available</h2>
        <button onClick={handleRetake} className="mt-4 px-8 py-3 bg-accent text-background rounded-full font-medium cursor-pointer">Return Home</button>
      </div>
    );
  }
  ...
  if (!report.assessment_summary) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center h-full min-h-screen p-6">
        ...
        <button onClick={handleRetake} className="mt-8 px-8 py-3 bg-accent text-background rounded-full font-medium cursor-pointer">Return Home</button>
      </div>
    );
  }
    ...
    <div className="flex-1 w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 pb-20">
            ...
            <div className="bg-surface/5 border border-surface/10 rounded-2xl p-6">
              ...
              <div className="mb-4">
                <span className={cn("px-4 py-1.5 rounded-full text-sm font-semibold uppercase tracking-wider border", getUnderstandingColor(assessment_summary.overall_understanding))}>
                  {(assessment_summary.overall_understanding || 'moderate').replace('_', ' ')}
                </span>
              </div>
          ...
          {topic_analysis && topic_analysis.length > 0 && (
            <>
              ...
              <div className="space-y-6 lg:max-h-[60vh] lg:overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-surface/20 scrollbar-track-transparent">
                {topic_analysis.map((topic: any, idx: number) => (
                  <div key={idx} className="bg-surface/5 border border-surface/10 rounded-2xl p-6 relative overflow-hidden group hover:border-surface/20 transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      <span className={cn("px-3 py-1 rounded-md text-xs font-semibold uppercase tracking-wider border", getUnderstandingColor(topic.understanding_level))}>
                        Level: {(topic.understanding_level || 'moderate').replace('_', ' ')}
                      </span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                       {topic.knowledge_gaps && topic.knowledge_gaps.length > 0 && (
                         <div>
                           ...
                           <ul className="space-y-2">
                             {topic.knowledge_gaps.map((g: string, i: number) => (
                       ...
                       {topic.misconceptions && topic.misconceptions.length > 0 && (
                         <div>
                           ...
                           <ul className="space-y-2">
                             {topic.misconceptions.map((m: string, i: number) => (
                               <li key={i} className="text-xs text-surface/70 flex items-start gap-1.5">
      ...
      <div className="mt-12 flex flex-col sm:flex-row items-center gap-4 border-t border-surface/10 pt-8">
        <button 
          ...
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3 rounded-full font-medium bg-accent text-background hover:bg-accent/90 transition-all focus:outline-none focus:ring-2 focus:ring-accent cursor-pointer"
        >
          <RefreshCw className="w-5 h-5" />
          Start New Interview
        </button>
        <button 
          className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3 rounded-full font-medium bg-transparent border border-surface/20 text-surface hover:bg-surface/10 transition-all focus:outline-none focus:ring-2 focus:ring-surface/30 cursor-pointer"
          onClick={() => {
            const dataStr = 'data:text/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(report, null, 2));
            const anchor = document.createElement('a');
            anchor.setAttribute('href', dataStr);
            anchor.setAttribute('download', `evaluator_report_${Date.now()}.json`);
