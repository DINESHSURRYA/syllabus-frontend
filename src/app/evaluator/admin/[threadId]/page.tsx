  ...
  useEffect(() => {
    async function fetchDetail() {
      ...
      try {
        ...
        const res = await fetch(`${baseUrl}/admin/interviews/${encodeURIComponent(threadId)}`);
  ...
  if (!detail) {
    return (
      <AppShell>
        <div className="min-h-screen relative flex bg-background text-surface">
          <ProgressThread />
          <main className="flex-1 p-8 text-center text-error">
            <p>Failed to load detail for thread {threadId}</p>
            <button onClick={() => router.push('/evaluator/admin')} className="mt-4 px-4 py-2 border rounded-full text-surface border-surface/20 cursor-pointer">
              Back to Admin
            </button>
          </main>
        </div>
        ...
        <main className="flex-1 ml-1 pl-4 sm:pl-8 flex flex-col min-h-screen">
          <div className="flex-1 w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 pb-20">
            <button 
              ...
              className="flex items-center gap-2 text-surface/70 hover:text-accent transition-colors mb-8 text-sm cursor-pointer"
            >
            ...
            <div className="space-y-12 mb-16 relative">
              ...
              {detail.interactions && detail.interactions.map((interaction: any, idx: number) => (
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                               {interaction.evaluation.concept_assessment.mastered?.length > 0 && (
                                 <div>
                                   ...
                                   <ul className="list-disc pl-4 text-xs text-surface/80">
                                     {interaction.evaluation.concept_assessment.mastered.map((c: string, i: number) => <li key={i}>{c}</li>)}
                                   </ul>
                                 </div>
                               )}
                               {interaction.evaluation.concept_assessment.misconceptions?.length > 0 && (
                                 <div>
                                   ...
                                   <ul className="list-disc pl-4 text-xs text-surface/80">
                                     {interaction.evaluation.concept_assessment.misconceptions.map((c: string, i: number) => <li key={i}>{c}</li>)}
                                   </ul>
                                 </div>
                               )}
                             </div>
                        ...
                        {interaction.evaluation?.evidence_ledger?.length > 0 && (
                          <div className="mb-4 bg-background p-4 rounded-xl border border-surface/10">
                             ...
                             <div className="space-y-4">
                               {interaction.evaluation.evidence_ledger.map((ev: any, i: number) => (
                                 <div key={i} className="text-xs border-l-2 border-accent pl-3">
                                   ...
                                   <ul className="list-disc pl-4 text-surface/70 space-y-1">
                                     {ev.supporting_observations?.map((obs: string, j: number) => <li key={j}>{obs}</li>)}
                                   </ul>
                                 </div>
                               ))}
                             </div>
